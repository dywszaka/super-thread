...
export function tmuxAttachCommand(session: Session, cwd: string): string {
  ...
  if (session.tmuxWindowKey) {
    ...
    return `if tmux has-session -t ${quotedSession} 2>/dev/null; then window=$(${lookup}); if [ -z "$window" ]; then window=$(tmux new-window -d -P -F '#{window_id}' -t ${quotedSession} -c ${quotedCwd}); fi; else window=$(tmux new-session -d -P -F '#{window_id}' -s ${quotedSession} -c ${quotedCwd}); fi; tmux set-option -w -t "$window" @superthread_terminal_id ${quotedKey}; status_left=$(tmux show-options -v -t ${quotedSession} status-left); status_left=$(printf '%s' "$status_left" | sed 's/#{session_name}/#{session_group}/g; s/#S/#{session_group}/g'); tmux kill-session -t ${clientSession} 2>/dev/null || true; tmux new-session -d -t ${quotedSession} -s ${clientSession}; tmux set-option -t ${clientSession} status-left "$status_left"; tmux select-window -t ${clientSession}:"$window"; exec tmux attach-session -t ${clientSession}`;
  }
  const quotedSession = quoteShellArgument(sessionName);
