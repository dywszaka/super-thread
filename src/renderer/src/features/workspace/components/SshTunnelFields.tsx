...
export function SshTunnelFields({ value, onChange }: { value: SshTunnelConfig[]; onChange(value: SshTunnelConfig[]): void }): ReactNode {
  ...
  const add = (): void => {
    const used = new Set(value.filter((tunnel) => tunnel.direction === "remote-to-local").map((tunnel) => tunnel.destinationPort));
    let port = 3000;
    while (used.has(port)) port += 1;
    onChange([...value, { direction: "remote-to-local", sourcePort: port, destinationHost: "127.0.0.1", destinationPort: port }]);
  };

  return (
    <section className="tunnel-section">
      <div className="tunnel-heading">
        <div><strong>SSH tunnels</strong><span>Direction means service location → listening location. Remote → Local makes a remote service available at localhost.</span></div>
      ...
      {value.map((tunnel, index) => {
        const sourceIsLocal = tunnel.direction === "local-to-remote";
        return (
          <div className="tunnel-card" key={`${index}-${tunnel.direction}`}>
            ...
            <div className="tunnel-grid">
              <Field label={sourceIsLocal ? "Local port" : "Remote port"}>
              ...
              <Field label={sourceIsLocal ? "Local host" : "Remote host"}>
              ...
              <Field label={sourceIsLocal ? "Remote port" : "Local port"}>
