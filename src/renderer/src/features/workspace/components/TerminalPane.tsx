...
function TerminalView({ session }: { session: Session }): React.ReactNode {
  ...
  useEffect(() => {
    ...
    const input = terminal.onData((data) => window.desktop.writeSession(session.id, data));
    const unsubscribe = window.desktop.onTerminalOutput((event) => { if (event.sessionId === session.id) terminal.write(event.data); });
